...
export default function AddLawyerModal({ onAdd, onClose, existingLawyers }: AddLawyerModalProps) {
  ...
  const validateForm = () => {
    ...
    if (existingLawyers.some(lawyer => lawyer.name.toLowerCase() === name.toLowerCase())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      const newLawyer = await lawyerService.createLawyer({
        name: name.trim()
      });
      onAdd(newLawyer);
    } catch (error) {
      console.error('Error creating lawyer:', error);
      setError('Error al crear la persona. Por favor, inténtalo de nuevo.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold text-gray-900">Agregar Persona</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-emerald-700 mt-1">
                  Agrega personas autorizadas para usar las laptops del sistema.
                </p>
              </div>
            </div>
          </div>
